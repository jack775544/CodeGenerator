...
namespace Generator.ProtectedRegions
{
	public static class ProtectedTemplateExtensions
	{
		...
		{
			var end = self.GetCurrentProtectedRegion().EndAsString();
		...
		internal static void AddProtectedRegion(this ITextTemplate self, ProtectedRegion region)
		{
			...
			if (self.Metadata.TryGetValue(ProtectedRegionMetadataKey, out var r))
			{
				regions = (List<ProtectedRegion>) r;
			}
			else
			{
				regions = new List<ProtectedRegion>();
				self.Metadata[ProtectedRegionMetadataKey] = regions;
			}
